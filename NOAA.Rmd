---
title: "U.S. storm data in relation to population health and property damage"
author: "Patrick Siu"
...
grid.arrange(fig2_a, fig2_b, nrow = 1)
...
```{r misc}
# noaa$Outliers <- "Year 1995 - 2011"
# noaa[noaa$Year < 1995, ]$Outliers <- "Year 1950 - 1994"
# noaa$Outliers <- as.factor(noaa$Outliers)
# 
# 
# noaa %>% group_by(EVTYPE) %>% summarize(propdamage = sum(propdamage)) %>% arrange(desc(propdamage))
# 
# noaa %>% filter(Year > year("1995")) %>% group_by(EVTYPE) %>% summarize(propdamage = sum(propdamage)) %>% arrange(desc(propdamage))




# 
# figure1 <- noaa %>% group_by(EVTYPE, Outliers) %>% summarize(propdamage = sum(propdamage)) %>% arrange(desc(propdamage))
# 
# ggplot(figure1, aes(x = EVTYPE, y = propdamage)) + geom_bar(stat = "identity") + factor(. ~ Outliers)
